...
class spawnpoint extends EventEmitter{
	 ...
	 */
	roundRobbin(items){
		return new require('./roundRobbin.js')(this)(items);
	}
	 ...
	 */
	getAndLock(items){
		return new require('./getAndLock.js')(this)(items);
	}
	 ...
	 * @param {String} The errorCode human readable Spawnpoint code.
	 * @param {Error} Instance of the error to map to..
	 * @return {this}
	 */
	...
	maskErrorToCode(error, type = 'code'){
		...
		_.each(self.errorMaps, (error, code) => {
			if(!returnedError && error instanceof error){
				returnedError = this[code](code, error);
			}
		});
	 ...
	 * This is step 1 of 8 to startup Spawnpoint on line #157
	 * @param  {String} [configFile]  Sets the JSON file Spawnpoint uses to setup the framework.
	 * @private
	 * @return {this}
	 */
	initConfig(configFile = null){
		...
		this.config.getRandom = function(path){
			let items = self.config.get(path);
			if(!items){
				throw new self._errorCode('app.config.sample_not_array'); // TODO: choose better name
			}
			return _.sample(items);
		};
		 ...
		 * @return {*} Returns random element from the collection.
		 */
		this.config.getRoundRobbin = function(path){
			if(!rrKeys[path]){
				let items = self.config.get(path);
