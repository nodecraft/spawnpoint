...
class spawnpoint extends EventEmitter{
	...
	setup(callback = () => {}){
		...
		if(this.status.setup){
			return callback(this.code('app.already_setup'));
		}
	...
	maskErrorToCode(error, type = 'code'){
		...
		_.each(this.errorMaps, (error, code) => {
	...
	initConfig(configFile = null){
		...
		this.config.getRandom = function(path){
			...
			if(!items){
				throw self.errorCode('app.config.sample_not_collection'); // TODO: choose better name
			}
			return _.sample(items);
		};
	 ...
	 */
	setupJSONHandler(){
		require(path.join(__dirname, '../require-extensions.js'));
		return this;
	}
}
